"""
Parser for GTFS Real-time JSON feeds
Extracts entities and stores in database instead of keeping JSON files
"""
import json
import sqlite3
    ...
    try:
        ...
        for entity in data.get('entity', []):
            ...
            c.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            ''', (
                f"{transport_type}_{vehicle_id}",  # FIXED: Use vehicle_id, not entity_id
                transport_type,
                vehicle_id,
                trip.get('trip_id'),
                trip.get('route_id'),
...
def parse_trip_updates(json_file, transport_type):
    ...
    try:
        ...
        for entity in data.get('entity', []):
            ...
            # Use trip_id as stable key, not timestamp-based entity_id
            ...
            c.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            ''', (
                f"{transport_type}_{trip_id}",  # FIXED: Use trip_id, not entity_id
                transport_type,
                trip_id,
                trip.get('route_id'),
                trip.get('direction_id'),
...
if __name__ == "__main__":
    ...
    metro_path = Path(__file__).parent / "metro"
    if metro_path.exists():
        # Test with latest files
