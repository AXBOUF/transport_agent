...
if response.status_code == 200:
    ...
    filename = f"vehicle_positions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    filepath = f"./realtime/sydneytrains/sydneytrains_pos/{filename}"
    with open(filepath, "w") as f:
        json.dump(data, f)
    print(f"✅ Vehicle positions saved to {filepath}")
else:
