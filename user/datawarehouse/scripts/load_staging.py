...
def create_and_load_table(conn, table_name: str, file_path: Path) -> None:
    ...
    if pk_cols:
        definitions.append(f"PRIMARY KEY ({', '.join(f'{c!r}' for c in pk_cols)})")
    conn.execute(text(f"""
...
def main() -> None:
    ...
    gtfs_source = os.getenv("GTFS_SOURCE", "gtfs_data")
    gtfs_dir = ROOT / "data" / gtfs_source
    db_name = os.getenv("POSTGRES_DB", "transport")

    print(f"Loading {gtfs_source} → {db_name}.staging")
    ...
    print(f"Done — staging load complete for {db_name}")
